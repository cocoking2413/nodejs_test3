 ...
 * @param {Function} callback
 */
exports.imgPath = function (url, callback) {
    debug('读取图片地址：%s', url);

    var sql = 'SELECT * FROM `img_list` AS `A`' +
        ' WHERE `A`.`url`=? LIMIT 1';
    db.query(sql, [url], function (err, list) {
        if (err) return callback(err);
        if (!(list.length > 0)) return callback(new Error('图片获取失败！'));
        console.log(list[0]);
        callback(null, list[0].path);
    });
};