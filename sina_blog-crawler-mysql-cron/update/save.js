            ...
            db.query('SELECT * FROM `class_list` WHERE `id`=? LIMIT 1', [item.id], function (err, data) {
                ...
                if (Array.isArray(data) && data.length >= 1) {
                   ...
                   // console.log(db.state);
                    // 分类不存在，添加
                    db.query('INSERT INTO `class_list`(`id`, `name`, `url`,`count`) VALUES (?, ?, ?,0)', [item.id, item.name, item.url], next);
                }
            });
 ...
 * @param {Function} callback
 */
exports.imgSave = function (url, path, callback) {
    debug('保存图片映射: %s', url);

    // 检查图片是否存在
    db.query('SELECT `id` FROM `img_list` WHERE `url`=?', [url], function (err, data) {
        if (err) return callback(err);

        if (Array.isArray(data) && data.length >= 1) {
            // 更新文章
            db.query('UPDATE `img_list` SET `path`=? WHERE `url`=?', [path,url], callback);
        } else {
            // 添加文章
            db.query('INSERT INTO `img_list`(`url`, `path`) VALUES (?, ?)', [url,path], callback);
        }
    });
};
 ...
 * @param {Function} callback
 */
exports.isImgExists = function (url, callback) {
    db.query('SELECT `id` FROM `img_list` WHERE `url`=?', [url], function (err, data) {
        if (err) return callback(err);
 ...
 * 清除数据表数据
 */
exports.clearTablesData = function (callback) {
    var desql=[];
    var errs=[];
    desql.push("DELETE  from  `img_list`");
    desql.push("DELETE  from  `article_list`");
    desql.push("DELETE  from  `article_detail`");
    desql.push("DELETE  from  `class_list`");
    desql.push("DELETE  from  `article_tag`");
    for(var item in desql){
        db.query(desql[item],function (err,result) {
