...
exports.articleDetail = function (url, callback) {
    ...
    request(url, function (err, res) {
        ...
        var $ = cheerio.load(res.body.toString());
        //console.log(1);
//fs.createWriteStream("../resouces/aaa_"+(new Date()).format("yyyyMMdd_hhmmssms")+".text",res.body.toString());
        // 获取文章标签
        var tags = [];
        ...
        $(".tagbox .BNE_lkC.lk_a").each(function () {
            var tag = $(this).text().trim();
            if (tag) {
                tags.push(tag);
            }
        ...
        console.log(url);
        console.log(typeof   $('.articalContent').html());
        console.log(typeof   $('.BNE_cont').html());
        // 获取文章内容
        var content = $('.articalContent').html();
        if (typeof content == "string") {console.log(0);
            content = content.trim();
        } else {console.log(1);
            content = $(".BNE_cont").html();
            if (typeof content == "string") {console.log(2);
                content = content.trim();
            } else {console.log(3);
